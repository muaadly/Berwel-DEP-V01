        ...
        complete: (results) => {
          ...
          const entries: MaloofEntry[] = results.data
            ...
            .map((entry: any, index: number) => {
              const likesCount = parseInt(entry['Likes'] || '0', 10);
              if (index < 5) { // Log for the first 5 entries to avoid excessive output
                console.log(`Maloof Entry ${index + 1} - Name: ${entry['Entry Name']}, Likes: ${likesCount}`);
              }
              return ({
